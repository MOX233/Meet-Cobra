...
def run_sim_withUMa(
    ...
    for x, frame_cur in enumerate(frame_list[1:]):
        ...
        # def see_micro_macro_diff(veh, fc_ghz=5.9, Gt_macro=0, NF=0, scenario='nlos'):
        #     PL = calculate_uma_pathloss_3gpp_38901(distance_2d_m=np.linalg.norm(pred_loc_dict[veh]-MacroBS_loc),
        #                                         fc_ghz=fc_ghz,
        #                                         h_ut_m=args.h_car,
        #                                         h_bs_m=args.h_tx,
        #                                         scenario=scenario)
        #     SE_macro = np.log2(1+calculate_channel_gain_from_pathloss(NF+PL-Gt_macro)*args.p_macro/(args.N0*args.RB_intervel_macro))
        #     SE_micro = np.log2(1+calculate_channel_gain_from_pathloss(NF-max(g_microBS_dict[veh]))*args.p_micro/(args.N0*args.RB_intervel_micro))
        ...
        # _ = see_micro_macro_diff(veh, fc_ghz=5.9, Gt_macro=17, NF=0, scenario='los')
        ...
        # 基于积压队列、预测位置、BS位置等信息进行PHO决策
        HO_cmd_cur4next = HO_func(
            args, veh_set_cur, Q_dict_cur, pred_loc_dict, pred_g_dict, BS_loc_array
        )
