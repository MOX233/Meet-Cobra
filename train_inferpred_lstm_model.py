...
if __name__ == "__main__":
    ...
    infer_g = np.zeros((num_sample,num_bs)).astype(np.float32)
    for veh in range(num_sample):
        for bs in range(num_bs):
            infer_g[veh,bs] = np.abs(channel[veh,:,bs,:]).max()
            infer_g[veh,bs] = 20 * np.log10(infer_g[veh,bs] + 1e-9) 
    infer_g_normalized = infer_g / 20 + 7
    infer_g_labels_np = infer_g_normalized[:,np.newaxis,...]
    data_torch, infer_g_labels, lengths = augment_dataset(
        data_np, infer_g_labels_np, look_ahead_len, augment_dataset_ratio=2)
    
    num_epochs =  100
    # 运行训练
    print('start training')
    best_model_weights, record_metrics = \
        train_gainpred_lstm_model(num_epochs, device, data_torch, lengths, infer_g_labels, M_t, M_r, pos_in_data=(preprocess_mode==2))
    save_file_name = f"inferpred_lstm_valMae{min(record_metrics['val_mae']):.2f}dB" \
