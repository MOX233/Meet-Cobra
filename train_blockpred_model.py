...
from utils.NN_utils import prepare_dataset, BlockPredictionModel, train_blockpred_model
...
if __name__ == "__main__":
    ...
    model, train_loss_list, train_acc_list, val_loss_list, val_acc_list = \
        train_blockpred_model(num_epochs, device, data_torch, veh_h_torch, M_t, M_r, pretrained_model_path, model_save_dir, pos_in_data=(preprocess_mode==2))
    train_result_name_list = split_string("model, train_loss_list, train_acc_list, val_loss_list, val_acc_list")
    
    save_name = f"blockpred_dimIn{model.feature_input_dim}_valAcc{max(val_acc_list):.2f}%"
