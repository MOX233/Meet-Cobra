...
from utils.NN_utils import BeamPredictionModel, train_blockpred_lstm_model
...
if __name__ == "__main__":
    ...
    block_labels_np = (veh_h_np[:,:,0,:,0]==0).astype(np.int64)
    data_torch, block_labels, lengths = augment_dataset(
        data_np, block_labels_np, look_ahead_len, augment_dataset_ratio=2)
    
    num_epochs =  100
    # 运行训练
    print('start training')
    best_model_weights, record_metrics = \
        train_blockpred_lstm_model(num_epochs, device, data_torch, lengths, block_labels, M_t, M_r, pos_in_data=(preprocess_mode==2))
    save_file_name = f"blockpred_lstm_valAcc{max(record_metrics['val_acc'])*100:.2f}%" \
