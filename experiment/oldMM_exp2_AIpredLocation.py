...
from utils.NN_utils import train_model, test_model, LSTM_Model_Mobility
...
if __name__ == "__main__":
    ...
    sim_save_path = (
        "/home/ubuntu/niulab/MM/experiment/exp_results/predLocation_"
    ...
    args.shd_sigma_macro = 4
    args.shd_sigma_micro = 4
    ...
    args.trajectoryInfo_path = (
        f"/home/ubuntu/niulab/MM/sumo_result/trajectory_Lbd{0.20:.2f}.csv"
    )
    ...
    save_path = "/home/ubuntu/niulab/MM/save"
    preprocess_params = np.load(
        os.path.join(save_path, f"preprocess/preprocess_params.npy"),
        allow_pickle=True,
    ).item()
    device = torch.device(
    ...
    save_path = os.path.join(
        save_path,
        f"model/shd[{args.shd_sigma_macro},{args.shd_sigma_micro}]fps[{ifps},{ofps}]/lstm[H{hidden_size}L{num_layers}D{drop_out}],mlp{mlp_hidden_dim_list}.npz",
    )
    model.load_state_dict(torch.load(save_path))

    # 给定各个基站的位置
    BS0_loc = np.array([0, 0])
    BS1_loc = np.array([250, 250])
    BS2_loc = np.array([250, -250])
    BS3_loc = np.array([-250, 250])
    BS4_loc = np.array([-250, -250])
    ...
    for data_rate_idx, data_rate in enumerate(data_rate_list):
        ...
        plt.savefig(os.path.join(sim_save_path, "exp2 Average system power.png"))
        plt.savefig(os.path.join(sim_save_path, "exp2 Average system power.pdf"))
        plt.close()
        ...
        plt.savefig(os.path.join(sim_save_path, "exp2 Average HO frequency.png"))
        plt.savefig(os.path.join(sim_save_path, "exp2 Average HO frequency.pdf"))
        plt.close()
        ...
        plt.savefig(os.path.join(sim_save_path, "exp2 Violation probability.png"))
        plt.savefig(os.path.join(sim_save_path, "exp2 Violation probability.pdf"))
        plt.close()
        ...
        plt.savefig(os.path.join(sim_save_path, "exp2 Average latency.png"))
        plt.savefig(os.path.join(sim_save_path, "exp2 Average latency.pdf"))
        plt.close()
        ...
        # 保存仿真实验结果指标
        np.save(os.path.join(sim_save_path, "sim_result_dict.npy"), sim_result_dict)
