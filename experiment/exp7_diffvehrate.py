...
if __name__ == "__main__":
    ...
    N_bs = 4
    freq = 28e9
    # DS_start, DS_end = 800, 950 # full test
    DS_start, DS_end = 800, 950 # quick test
    preprocess_mode = 0
    ...
    args.NF_macro_dB = 5
    args.NF_micro_dB = 10
    args.data_rate = 50 * 1e6 # 50 Mbps
    args.random_factor_range4data_rate = 0.0
    args.lat_slot_ub = 20 # latency threshold 20 ms
    args.eta = 1e6
    args.device = device
    args.K = 3 # 每次beam tracking 时选K个最有可能的波束对进行测试
    
    Lambda_list = np.linspace(0.1, 1.5, 15) # 车辆到达率
    
    # 对测试数据集进行截断
    cut_ratio = 1
    cut_end = DS_start + cut_ratio*(DS_end-DS_start)
    save_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), f"results_exp7/datarate{args.data_rate:.2e}_{DS_start}_{cut_end}_"
    ...
    # 进行仿真实验
    for veh_rate_idx, veh_rate in enumerate(Lambda_list):
        args.Lambda = veh_rate
        ...
        if os.path.exists(data4sim_filepath):
            ...
            if not os.path.exists(sionna_result_filepath):
                run_sionna_sim(
                    args=args,
                    start_time=DS_start, end_time=DS_end, 
                    subprocess_time=1, 
                    max_workers=5, # 最大线程数
                    timeout=3600, # 子进程超时时间（秒）
                    gpu=gpu, # GPU编号
                    Lambda=veh_rate, #车辆到达率
                    freq=args.freq, # 28e9 or 5.9e9
                    antenna_pattern="iso", # "iso" or "tr38901"
                    N_t_H=1, N_r_H=1,
                    N_t_V=args.M_t, N_r_V=args.M_r,
                    h_car=1.6, h_rx=3, h_tx=35,
                    )
            print(f'load sionna result from {sionna_result_filepath}')
        ...
        print(f"vehicle arrival rate: {veh_rate:.1f} veh/s, average vehicle number: {avg_car_num:.1f}")
        for strategy_name in sim_strategy_dict.keys():
        ...
        for strategy_name in sim_strategy_dict.keys():
            plt.plot(
                Lambda_list[: veh_rate_idx + 1],
                np.array(sim_result_dict[strategy_name]["HOps_list"][: veh_rate_idx + 1]) / avg_car_num,
                "*-",
                label=strategy_name,
            )
        plt.legend()
        ...
        for strategy_name in sim_strategy_dict.keys():
            plt.plot(
                Lambda_list[: veh_rate_idx + 1],
                sim_result_dict[strategy_name]["vio_prob_list"][: veh_rate_idx + 1],
                "*-",
                label=strategy_name,
            )
        plt.legend()
        ...
        for strategy_name in sim_strategy_dict.keys():
            plt.plot(
                Lambda_list[: veh_rate_idx + 1],
                sim_result_dict[strategy_name]["avg_latency_list"][: veh_rate_idx + 1],
                "*-",
                label=strategy_name,
            )
        plt.legend()
        ...
        for BS_id in range(len(BS_loc_list)+1):
            ...
            for strategy_name in sim_strategy_dict.keys():
                avg_carnum_under_BS_list = np.array(sim_result_dict[strategy_name]["carnum_under_BS_list"][: veh_rate_idx + 1]).mean(axis=-2)
                plt.plot(
                    Lambda_list[: veh_rate_idx + 1],
        ...
        sim_result_dict["Lambda_list"] = Lambda_list
        # 保存仿真实验结果指标
