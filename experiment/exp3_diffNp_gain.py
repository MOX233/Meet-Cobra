...
from utils.NN_utils import BeamPredictionModel, train_beampred_lstm_model, train_gainpred_lstm_model
from utils.options import args_parser
...
save_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), f"results_exp3/lbd{lbd:.2f}_datasize{data_size_test}_gainpred_"
...
for Np in Np_list:
    ...
    best_model_weights, record_metrics = \
        train_gainpred_lstm_model(num_epochs, device, data_torch_test[:,:,::int(n_pilot/Np)], lengths_test, gain_labels_test, M_t, M_r, pos_in_data=(preprocess_mode==2))
...
def get_recored_bestKPIs(record_metrics_list, Np_list=[1,2,4,8,16,32]):
    ...
    for Np, record_metrics in zip(Np_list, record_metrics_list):
        ...
        best_epoch = np.argmin(record_metrics['val_rmse'])
        bestKPI_dict[Np]['best_epoch'] = best_epoch + 1
        bestKPI_dict[Np]['best_train_rmse'] = record_metrics['train_rmse'][best_epoch]
        bestKPI_dict[Np]['best_val_rmse'] = record_metrics['val_rmse'][best_epoch]
        bestKPI_dict[Np]['best_train_mae'] = record_metrics['train_mae'][best_epoch]
        bestKPI_dict[Np]['best_val_mae'] = record_metrics['val_mae'][best_epoch]
    return bestKPI_dict

# 修改get_recored_bestKPIs
